...
public:
    ...
    // This function was added by Sarah Lutteropp
    intervalVector findCovering(K start, K stop) const {
	intervalVector covering;
	this->findCovering(start, stop, covering);
	return covering;
    }

    void findCovering(K start, K stop, intervalVector& covering) const {
        if (!intervals.empty() && ! (stop < intervals.front().start)) {
            for (typename intervalVector::const_iterator i = intervals.begin(); i != intervals.end(); ++i) {
                const interval& interval = *i;
                if (interval.start <= start && interval.stop >= stop) {
        ...
        if (left && start <= center) {
            left->findCovering(start, stop, covering);
        }

        if (right && stop >= center) {
            right->findCovering(start, stop, covering);
        }
